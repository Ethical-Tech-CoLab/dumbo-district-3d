...
function lightness(base: THREE.Color, ndl: number, hemiFactor: number, rig: ReturnType<typeof skyLighting>, normal: THREE.Vector3) {
  const sun = base.clone().multiply(new THREE.Color(rig.sunColour)).multiplyScalar(rig.sunIntensity * ndl);
  const hemi = base.clone().multiply(new THREE.Color(rig.hemiSky)).multiplyScalar(rig.hemiIntensity * hemiFactor);
  ...
  const total = sun.add(hemi).add(fill);
...
function row(label: string, altitudeDeg: number, azimuthDeg: number) {
  ...
  const litWall = lightness(BRICK, Math.max(0, facing.dot(dir)), 0.5, rig, facing);
  const shadeWall = lightness(BRICK, Math.max(0, away.dot(dir)), 0.5, rig, away);
  const groundNdl = Math.max(0, up.dot(dir));
  const pavement = lightness(CONCRETE, groundNdl, 1.0, rig, up);
  const road = lightness(ASPHALT, groundNdl, 1.0, rig, up);
  ...
  const bestWall = lightness(BRICK, Math.max(0, best.dot(dir)), 0.5, rig, best);
  ...
  return { best: bestWall, shade: shadeWall, pave: pavement };
}
